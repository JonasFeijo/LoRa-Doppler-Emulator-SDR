  ...
  namespace lora_sdr {
    ...
    class modulate_impl : public modulate
    {
     private:
        ...
        float m_doppler_shift;
        float m_doppler_rate;
        ...
        float os_factor_doppler;
        float os_factor_doppler_rate;
        float offset_integration;
        float m_doppler_offseted;
     ...
     public:
      modulate_impl(uint8_t sf, uint32_t samp_rate, uint32_t bw, std::vector<uint16_t> sync_words, uint32_t frame_zero_padd, uint16_t preamb_len, int doppler_shift, int doppler_rate);
      ~modulate_impl();
