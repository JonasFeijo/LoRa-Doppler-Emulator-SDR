...
namespace gr {
    namespace lora_sdr {
         ...
         */
        inline void build_upchirp(gr_complex* chirp, uint32_t id, uint8_t sf, uint8_t os_factor = 1, float os_factor_doppler=0,float os_factor_doppler_rate=0){
            double N = (1 << sf)  ;
            int n_fold = N* os_factor - id*os_factor;

            //std::cout << "os_factor: " << static_cast<float>(os_factor_doppler_rate) << std::endl;

            for(int n = 0; n < N* os_factor; n++){
                if(n<n_fold)
                    chirp[n] = gr_complex(1.0,0.0)*gr_expj(2.0*M_PI *(n*n/(2*N)/pow(os_factor,2)+(n*n*os_factor_doppler_rate/2)+(id/N-0.5)*n/os_factor+(n*os_factor_doppler)));
                else
                    chirp[n] = gr_complex(1.0,0.0)*gr_expj(2.0*M_PI *(n*n/(2*N)/pow(os_factor,2)+(n*n*os_factor_doppler_rate/2)+(id/N-1.5)*n/os_factor+(n*os_factor_doppler)));
         ...
         *          The spreading factor to use
         */
        inline void build_ref_chirps(gr_complex* upchirp, gr_complex* downchirp, uint8_t sf, uint8_t os_factor = 1, float os_factor_doppler=0,float os_factor_doppler_rate=0){
            double N = (1 << sf);
            build_upchirp(upchirp,0,sf,os_factor,-os_factor_doppler,os_factor_doppler_rate);
